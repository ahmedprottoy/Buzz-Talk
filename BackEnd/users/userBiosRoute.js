...
userBios.updateProfile = async (req, res, next) => {
  ...
  db.query(searchSQL, [userId],async (err, result) => {
    if (err) {
      ...
      const profileUpdate = "Update userinfo set userName = ? , firstName = ? , lastName = ? , email = ?,  password = ? where userId = ?";

      db.query(profileUpdate,[userName, firstName, lastName, email, hashedPassword, userId],(err1 , rows)=>{

        if(err1){
          next(err1);
        }
        else{
          const biosUpdate = "Update userbios set location = ? , profession = ? , religion = ? where userId = ?";

          db.query(biosUpdate,[location, profession, religion, userId], (err2,result) => {
            if(err2){
              next(err2);
            }
            else{
              res.status(200).json({ message:"You have successfully updated your profile" });
            }
          })
        }
      });
...
userBios.getProfile = (req, res, next) => {
  const userId = req.user.id;
  const searchSQL = "SELECT userName, firstName, lastName, email, location, profession, religion FROM socialmedia.userbios, socialmedia.userinfo where userbios.userId=userinfo.userId and userinfo.userId = ?";
