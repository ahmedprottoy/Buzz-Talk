...
followHandler.startFollow = (req, res, next) => {
    ...
    const insertQuery = "insert into socialmedia.follower_table( userID, followerID ) values( ?, ?)";
  
    db.query(insertQuery, [userID, followerID], (err, results) => {
      if(err){
        next(err);
      } else {
        res.status(200).json({message:`Congrats! You just start following ${userID}`});
        console.log(results);
      }
    });
  ...
  followHandler.getFollowing = (req, res, next) => {
    ...
    from socialmedia.userbios,(select *
    from socialmedia.userinfo
    where userinfo.userID in  (select userID from socialmedia.follower_table where followerID = 4)) as tb
    where userbios.userId = tb.userID`;
  ...
  followHandler.unfollow = (req, res, next) => {
    ...
    const deleteQuery = "DELETE FROM socialmedia.follower_table WHERE userID = ? and followerID = ?;";
