...
commentHandler.postComment = (req, res, next) => {
    ...
    if(commentImg == "null" && commentText == "null"){
      ...
      db.query(insertQuery, [userID, postID, commentText, commentImg], (err, results) => {
        if (err) {
          next(err);
        } else {
          res.json({
...
commentHandler.getComment = (req, res, next) => {
    const postID = req.params.postID;
    const searchQueary = `SELECT commentID , userName , commentText, commentImg FROM socialmedia.comment_table, socialmedia.userinfo
    where comment_table.userID = userinfo.userID and postID = ?;`;
